...
def initialize_model(data):
    ...
    model.GenMix_Target = Param(initialize=0.5, mutable=True)  
    ...
    def objective_rule(model):
        ...
        return capex_n_fixed_costs + variable_costs
    ...
    def genmix_share_rule(model):
        ...
        # effective_share = max(epsilon, (1 - value(model.GenMix_Target)))
...
def run_solver(model, log_file_path='solver_log.txt', mipgap=0, num_runs=1):
    ...
    for run in range(num_runs):
        ...
        result = solver.solve(model, tee=True, keepfiles=True, logfile=log_file_path, options_string="threads=4", timelimit=7200)
...
def main():
    ...
    for j in iso:
        for i in nuclear:
            case = f"{j}_Nuclear_{i}_VariedTargets"
