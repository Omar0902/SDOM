...
def initialize_model(data):
    ...
    model.EUE_max = Param(initialize=10, mutable=True)  # Maximum EUE (in MWh)
...
def run_solver(model, log_file_path='solver_log.txt', mipgap=0.03, num_runs=1):
    ...
    for run in range(num_runs):
        ...
        result = solver.solve(model, tee=True, keepfiles=True,
                              logfile=log_file_path, options_string="threads=8")
        ...
        for i in nuclear:
            for k in target:
                ...
                if best_result:
                    ...
                    print(f"Solver did not find an optimal solution for {
                          case}, skipping result export.")
