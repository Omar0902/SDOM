...
from resilience_scenarios import generate_resilience_scenarios

# ---------------------------------------------------------------------------------
import pandas as pd
import os

def load_data(base_dir, scenario=None):
    # Load datasets and process columns properly
    cf_solar = pd.read_csv(os.path.join(base_dir, "CFSolar_2050.csv")).round(5)
    cf_solar.columns = cf_solar.columns.astype(str)

    cf_wind = pd.read_csv(os.path.join(base_dir, "CFWind_2050.csv")).round(5)
    cf_wind.columns = cf_wind.columns.astype(str)

    cap_solar = pd.read_csv(os.path.join(base_dir, "CapSolar_2050.csv")).round(5)
    cap_solar['sc_gid'] = cap_solar['sc_gid'].astype(str)

    cap_wind = pd.read_csv(os.path.join(base_dir, "CapWind_2050.csv")).round(5)
    cap_wind['sc_gid'] = cap_wind['sc_gid'].astype(str)

    # Dictionary to hold all the loaded data
    base_data = {
        "solar_plants": pd.read_csv(os.path.join(base_dir, "Set_k_SolarPV.csv"), header=None)[0].tolist(),
        "wind_plants": pd.read_csv(os.path.join(base_dir, "Set_w_Wind.csv"), header=None)[0].tolist(),
        "cf_solar": cf_solar,
        "cf_wind": cf_wind,
        "cap_solar": cap_solar,
        "cap_wind": cap_wind,
        "storage_data": pd.read_csv(os.path.join(base_dir, "StorageData_2050.csv"), index_col=0).round(5),
    }

    # Load demand data (scenario or default)
    if scenario is not None:
        base_data["load_data"] = scenario.copy()
    else:
        base_data["load_data"] = pd.read_csv(os.path.join(base_dir, "Load_hourly_2050.csv")).round(5)


    # Extract nuclear, hydro, and renewables data separately
    base_data["nuclear_data"] = base_data["load_data"][["*Hour", "Nuclear"]].copy()
    base_data["large_hydro_data"] = base_data["load_data"][["*Hour", "LargeHydro"]].copy()
    base_data["other_renewables_data"] = base_data["load_data"][["*Hour", "OtherRenewables"]].copy()
...
def initialize_model(data):
    ...
    model.EUE_max = Param(initialize=5, mutable=True)  # Maximum EUE (in MWh)
    ...
    critical_load_percentage = 0.1  # 10% of the total load
    ...
    def genmix_share_rule(model):
        ...
        total_demand = sum(model.Load[h]
                           ...
                           - sum(model.PD[h, j] for j in model.j)
                           for h in model.h)
        return total_gas <= (1 - model.GenMix_Target)*total_demand
...
def run_solver(model, log_file_path='solver_log.txt', mipgap=0, num_runs=1):
    solver = SolverFactory(
        'cbc', executable='/opt/anaconda3/lib/python3.12/site-packages/pulp/solverdir/cbc/osx/64/cbc')
    ...
    for run in range(num_runs):
        ...
        result = solver.solve(model, tee=True, keepfiles=True,
                              logfile=log_file_path, options_string="threads=4")
...
def main():
    """Runs the optimization model for different resilience scenarios."""
    
    base_dir = os.path.dirname(os.path.abspath(__file__))  
    scenarios = generate_resilience_scenarios(base_dir)

    iso_regions = ["CAISO"]
    nuclear_levels = ["1"]
    targets = ["0.9"]

    for iso in iso_regions:
        for nuclear in nuclear_levels:
            for target in targets:
                for scenario_name, scenario_data in scenarios.items():
                    print(f"Running optimization for {iso}, {scenario_name}, Nuclear={nuclear}, Target={target}...")
                    
                    # Load modified data with outage scenarios
                    data = load_data(base_dir, scenario_data)
                    model = initialize_model(data)
                    
                    results, best_result = run_solver(model)

                    if best_result:
                        print(f"Best result for {iso}, {scenario_name}: {best_result}")
                        export_results(model, iso, scenario_name)
                    else:
                        print(f"Solver did not find an optimal solution for {iso}, {scenario_name}, skipping result export.")
