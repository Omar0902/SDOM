...
from .models_utils import add_generation_variables, add_alpha_and_ts_parameters, add_upper_bound_paramenters, add_lower_bound_paramenters
...
def add_large_hydro_bound_parameters(model, data):
    ...
    add_upper_bound_paramenters(model.hydro, model.h, data, "AlphaLargHy", "large_hydro_max", "LargeHydro")
    add_lower_bound_paramenters(model.hydro, model.h, data, "AlphaLargHy", "large_hydro_min", "LargeHydro")
...
def hydro_budget_rule(block, hhh):
...
def add_hydro_budget_constraints(model, data):
    model.hydro.budget_constraint = Constraint(model.hydro.budget_set, rule = hydro_budget_rule )
