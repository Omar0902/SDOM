...
# ----------------------------------- Parameters -----------------------------------|
####################################################################################|
def add_pv_parameters(model, data):
    ...
    for property_name in VRE_PROPERTIES_NAMES:#['trans_cap_cost', 'CAPEX_M', 'FOM_M']:
        ...
        default_value = 0.0
        filtered_property_dict_solar = {k: property_dict_solar.get(k, default_value) for k in model.pv.plants_set}
        model.add_component(f"CapSolar_{property_name}", Param(model.pv.plants_set, initialize=filtered_property_dict_solar))

    model.CapSolar_capacity = Param( model.pv.plants_set, initialize = filtered_cap_solar_dict )

    # Solar capacity factor initialization
    cf_solar_melted = data["cf_solar"].melt(id_vars='Hour', var_name='plant', value_name='CF')
    cf_solar_filtered = cf_solar_melted[(cf_solar_melted['plant'].isin(model.pv.plants_set)) & (cf_solar_melted['Hour'].isin(model.h))]
    cf_solar_dict = cf_solar_filtered.set_index(['Hour', 'plant'])['CF'].to_dict()
    model.CFSolar = Param( model.h, model.pv.plants_set, initialize = cf_solar_dict )

def add_wind_parameters(model, data):
...
def add_vre_parameters(model, data):
    add_wind_parameters(model, data)
    add_pv_parameters(model, data)
...
def add_vre_variables(model):
    ...
    model.Ypv = Var(model.pv.plants_set, domain=NonNegativeReals, bounds=(0, 1), initialize=1)
...
def solar_fixed_om_cost_expr_rule(model):
    ...
    """
    return sum( ( MW_TO_KW * model.CapSolar_FOM_M[k]) * model.CapSolar_capacity[k] * model.Ypv[k] for k in model.pv.plants_set )
...
def add_vre_expressions(model):
    ...
    model.total_hourly_pv_plant_availability = Expression(model.h, model.pv.plants_set, rule=lambda model, h, k: model.CFSolar[h, k] * model.CapSolar_capacity[k] * model.Ypv[k])
...
def solar_balance_rule(model, h):
    return model.total_hourly_pv_availability[h] == sum(model.total_hourly_pv_plant_availability[h, k] for k in model.pv.plants_set)
